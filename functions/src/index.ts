...
async function buildSnapshotPdf(week: WeekRange): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(MUTED).fontSize(10)
      .text(`${week.weekLabel}  ·  ${week.dateRange}  ·  Generated automatically by Antrac ERP`, 48, 50);
    ...
    doc.fillColor(MUTED).fontSize(8).font('Helvetica')
      .text(
        `Generated by Antrac ERP on ${new Date().toISOString()} UTC.\n` +
...
export const weeklyOpsSnapshot = onSchedule(
  ...
  async () => {
    ...
    await db.collection('documents').add({
      ...
      uploadedById: 'system',
      uploadedByName: 'Antrac ERP (auto)',
